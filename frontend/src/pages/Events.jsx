...
import { Calendar, MapPin, BookOpen, GraduationCap, Briefcase, Search } from 'lucide-react'
...
export default function Events() {
  ...
  useEffect(() => {
    setLoading(true)
    api.get('/events').then(d => setEvents(d || [])).catch(() => setEvents([])).finally(() => setLoading(false))
  }, [])
  ...
  return (
    <main>
      <PageHeader
        ...
        subtitle={isRtl ? 'فعاليات وندوات ومشاريع وبرامج تدريبية' : 'Events, seminars, projects and training programs'}
      >
                  ...
                  <div key={item.id} className="card-hover bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden group">
                    {item.image_url && (
                      <div className="relative overflow-hidden h-44">
                        ...
                        <span className={`absolute top-3 start-3 text-xs font-semibold px-2.5 py-1 rounded-full border ${colorClass}`}>
                          {typeLabel}
                        </span>
                      </div>
                    )}
                      ...
                      <div className="flex items-center gap-4 text-xs text-gray-500 mb-3">
                        {item.event_date && (
                          <span className="flex items-center gap-1">
                            ...
                            {new Date(item.event_date).toLocaleDateString(isRtl ? 'ar-YE' : 'en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
                          </span>
                        )}
                        {item.location && (
                          <span className="flex items-center gap-1">
